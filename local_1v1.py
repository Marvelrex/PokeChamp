...
def valid_player_name(value: str):
    allowed = ['pokechamp', 'pokellmon', 'one_step', 'abyssal', 'max_power', 'random']
    for a in allowed:
        if value == a or value.startswith(a):
            return value
    raise argparse.ArgumentTypeError(
        f"invalid choice: {value!r} (must start with one of {allowed})"
    )
...
parser.add_argument("--player_backend", type=str, default="gpt-4o", choices=["gpt-4o-mini", "gpt-4o", "gpt-4o-2024-05-13", "llama3.1:8b", 'gemma:7b', 'gemma:2b' ,'None'])
parser.add_argument("--player_name", type=valid_player_name, default='pokellmon')
...
parser.add_argument("--use_prediction_engine", type=str,default="False", choices=["high", "low"])

# Opponent arguments
parser.add_argument("--opponent_prompt_algo", default="io", choices=prompt_algos)
parser.add_argument("--opponent_backend", type=str, default="gpt-4o", choices=["gpt-4o-mini", "gpt-4o", "gpt-4o-2024-05-13", "llama3.1:8b", 'gemma:7b', 'gemma:2b' ,'None'])
...
parser.add_argument("--opponent_name", type=valid_player_name, default='pokellmon')
parser.add_argument("--opponent_device", type=int, default=0)
parser.add_argument("--opp_use_prediction_engine", type=str,default="False", choices=["high", "low"])
...
async def main():
    player = get_llm_player(args, 
                            ...
                            use_prediction_engine=args.use_prediction_engine)
    ...
    opponent = get_llm_player(args, 
                            ...
                            PNUMBER1=PNUMBER1,  # for name uniqueness locally
                            battle_format=args.battle_format,
                            use_prediction_engine=args.opp_use_prediction_engine)
